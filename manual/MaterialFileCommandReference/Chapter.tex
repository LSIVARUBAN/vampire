...
Defines the fourth order N\'eel pair anisotropy coefficient ($q(r_{ij})$) in the fourth order N\'eel pair anisotropy model for collinear spins. Both \textit{material:neel-anisotropy-constant} ($\ell(r_{ij})$) and
\textit{material:neel-fourth-order-anisotropy-constant} ($q(r_{ij})$) must be specified for the fourth order model.
