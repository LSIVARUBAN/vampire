...
namespace anisotropy {
   namespace internal {
      ...
      // To do: add new array for rank 4 tensor in internal.hpp
      // std::vector<double> neel_tensor_4; Size should be 81 * num_atoms

      //---------------------------------------------------------------------------------
      // Function to add Full fourth order Neel anisotropy for magnetite Fe - O surface interactions
      //
      // Model derived from Eq 6 of:
      // Nieves, P., et al. "Spin-lattice model for cubic crystals." Phys. [cite_start]Rev. B 103, 094437 (2021)[cite: 69].
      //
      // Hamiltonian:
      ...
      //    Q_eff = q(r)
      //
      // Tensor Definitions (to be calculated in initialisation):
      ...
      void neel_fields_fourth(std::vector<double>& spin_array_x,
        ...
        // loop over all atoms
         for(int atom = start_index; atom < end_index; atom++){
            ...
            // Field update is used for every atom so we need to reset it here.
